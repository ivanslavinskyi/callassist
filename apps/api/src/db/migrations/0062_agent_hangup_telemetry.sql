-- Additive event vocabulary; existing immutable event rows are never rewritten.
...
ALTER TABLE call_events ADD CONSTRAINT call_events_event_name_check CHECK (event_name IN (
  ...
  'conversation.hangup', 'transcription.started', 'transcription.completed',
  'transcription.failed', 'call.recovered'
));
