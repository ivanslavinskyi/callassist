# Historical roadmap snapshot ? 2026-09-12

Archived on 2026-09-13 before consolidating the current backlog. The original working-copy text follows unchanged. Status statements below are historical; in particular, the dependency gate has reopened. Use [the current roadmap](mvp-plan.md) and [the fresh audit](public-testing-audit-2026-09-13.md) for release decisions.

---

# SHPROHLI release roadmap

Reviewed 2026-09-12 against code baseline `ef36cfa`. The language workflow, appointments,
conversation stabilization and general result/live-transcript fixes are committed.
See the [documentation index](README.md) for dated audits and verification; the
2026-09-07 merge report describes that earlier integration only.
...
R01-R05 and R18/R19 are implemented and verified locally. R06's async CLI preparation
has a non-billable harness, but its start stage lacks v2 review evidence and is not
complete for new plans. Supervised calls through the UI have been recorded,
including hangup and a confirmed personal meeting; the complete current-commit
provider/outage matrix remains open.
...
### Delivered after the design milestone

| Package | Current implementation | Remaining acceptance |
| --- | --- | --- |
| W00–W10 languages/result/landing | Independent UI/task/call languages, detection-first task language with pre-approval correction, exact original/translation receipts, plan/clarification translation, transcript translation, automatic summaries, extensible UI/CMS and educational demo | UI remains EN/DE; 7–15 dictionaries are not delivered. Real text directions need explicit enablement and pair-specific acceptance. CMS candidates/drafts require reconciliation/publication; seed copy is not proof of publication |
| Appointment extension | One explicitly authorized booking or confirmation, finite local time windows, server check_appointment, recipient-confirmed outcome | Multilingual/adversarial and organization/confirm-existing voice matrix; no calendar API, rescheduling, cancellation or new financial terms |
| C01–C06 conversation/result stabilization | Silent runtime controls, interrupted-farewell decision, shorter new plans, schema-2 compact summary/findings, minimized tool telemetry (0068), local summary cutover | New spoken instructions and interrupted farewell still need current-code audio acceptance; no general removal of plan/approval/encryption compatibility was performed |
| A01–A03 general fixes | Durable retry/status/budget handling across text kinds; coherent live delta/final/discard identity; reconnect/snapshot guards; intent-based following; general task speech rules | Full live/device acceptance remains; six fictional real-text scenarios and the 11-check browser follower fixture are dated evidence, not exhaustive model coverage |

Sources: [language delivery](verification-language-implementation-2026-09-09.md),
[UI simplification](ui-language-simplification-2026-09-09.md),
[appointment delivery](unified-implementation-plan-2026-09-09.md#10-расширение-от-10092026--одна-запись-по-предварительному-разрешению),
[conversation delivery](call-conversation-result-stabilization-plan-2026-09-10.md),
[general fixes and verification](call-result-live-fixes-2026-09-11.md).

### Next work, in order

1. Complete supervised voice acceptance on the current code: consent, information
   and negative/partial answers, booking/confirm-existing, recipient refusal/hangup,
   interrupted farewell, silent tool use and corresponding final result.
2. Reconcile the existing CMS drafts with current source copy, including appointments
   and the short spoken notice; review and publish through the normal CMS workflow (R07).
3. Complete broader browser/device/accessibility and language-pair coverage (R13/R14).
4. Prepare and exercise the target deployment, recovery, operations and release gates
   (R09–R12); dictionaries beyond EN/DE remain separate product delivery.
...
| Preparation | Async `202` creation/edit/clarification, stable owner idempotency, bounded deadline/retry, durable compilation, fenced publication and source-input erasure | [preparation schema](../packages/contracts/src/call-preparation.ts), [runtime](runtime-reference.md) |
...
| Storage/tooling | 68 checksummed migrations through 0068, memory/PostgreSQL repositories, versioned encryption, seventeen-column rotation verifier, backup/restore tool, CI source and Dependabot | [architecture](architecture.md), [recovery](database-recovery-and-secrets.md) |
...
## Delivered product follow-up — R21: favicon, represented names and agent hangup
...
- Supplied SHPROHLI SVG integrated as a favicon in the existing dark/emerald palette.
- New plans default to current account first/last names in directly editable fields
  without an unlock button; preserve stored plan values and immutable approval/recompilation.
- Implemented structured agent termination, farewell playback confirmation, interruption
  handling and bounded provider fallback with correct attempt/status accounting.

The original delivery order is complete. Later interrupted-closing and speech
changes are recorded under C01–C06/A01–A03 above; their voice acceptance remains explicit.
...
| R02 / P0 | Implemented — platform/backend | Declare Turbo runtime/test/build env and cache inputs; prevent silent DB-suite skipping in CI | On a clean checkout without `.env`, injected DB URL reaches all applicable PostgreSQL suites; unavailable DB fails the gate; API/worker/web receive intended env; changes to build origins invalidate cache |
...
| R06 / P0 | Partial — backend/QA | Async prepare/poll/read exists, but CLI start omits required v2 review evidence. Adapt it to the explicitly reviewed original/translation and selection revision; preserve existing-account-only and worker-ordering rules | Test against real API/repository authorization, including missing/stale/foreign review rejection; then complete an authorized current-code provider/outage drill. The fake-server harness alone cannot establish receipt compatibility; current calls use the UI |
| R07 / P0 | Partial — product/privacy/backend | Source copy, educational demo, short spoken consent and candidate/staging tooling exist. Reconcile the dated unpublished drafts with current appointment/privacy/retention/Resend copy and complete editorial publication | Verify actual published EN/DE revisions through the CMS, including preserved AUP/Landing drafts; capture copy/consent evidence on the release candidate. Spoken notice retains AI identity, represented person and recording/transcription consent. No source-code update alone establishes CMS publication |
| R08 / P0 | Partial — backend/safety | Immutable plans, exact review receipts, fenced attempts, full-plan moderation and one explicitly authorized booking/confirmation are implemented. Preapproved facts may be used as needed; additional live permission prompts are excluded. One supervised personal-meeting success is recorded. Rescheduling, cancellation, payment, deposits and new financial/cancellation terms remain prohibited | Complete adversarial/multilingual checks for approved facts, time windows, false confirmations, interrupted closing and recipient stop. Verify silent tool use and current instructions against audio. A successful permission check is not a booking; isolated live success does not close the matrix |
...
R06 has a tested prepare/poll/read stage; its separate start stage still needs the
v2 review-receipt contract update identified during documentation reconciliation.
...
compilation, Realtime, consent recognition, telephony, post-call ASR and text artifacts. SMS/email
...
- Additional UI dictionaries beyond EN/DE and their CMS/device/RTL acceptance.
  Editable account-name defaults, persisted language preferences and the manual
  theme toggle are already implemented.
...
Unrestricted identity, automatic selection of arbitrary call languages, international expansion and
...
- [ ] Fresh full-suite/CI, lint/typecheck/build and applicable recovery evidence on
  the candidate release commit. Dated local evidence is indexed in [docs](README.md);
  successful later targeted checks do not imply a rerun of the complete suite.
