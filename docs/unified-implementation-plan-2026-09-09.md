...
Создан 09.09.2026 на базе `7b86d3a`; статус сверён 12.09.2026 с `ef36cfa`. Текущий каталог — 68 миграций, до `0068_conversation_tool_results.sql`.

Статус: **код W00–W08 и локальная часть W09/W10 сохранены в Git; расширение встреч и C01–C06/A01–A03 также реализованы. Публикация и отдельные проверки выпуска остаются открытыми**. Фактические тесты, миграции и ограничения записаны в [verification](verification-language-implementation-2026-09-09.md). Галочки ниже относятся к реализации, а не к развёртыванию или доказанному качеству всех языковых пар.

Это запись принятых решений и поставки, объединяющая [расхождения обещаний и реализации](promise-implementation-gap-plan-2026-09-08.md) и [языковую архитектуру](language-workflow-architecture-2026-09-09.md). Эти два документа сохраняются как основания решений. [Аудит](landing-audit-2026-09-08.md) — историческое наблюдение; [release roadmap](mvp-plan.md) — источник условий публичного выпуска.
...
Зафиксированные ограничения: UI первоначально остаётся EN/DE; готовим подключение 7–15 языков без изменения call domain. Переводы этих 7–15 словарей — отдельные последующие поставки. Дополнительные разрешения во время звонка, перевод живого аудиопотока, новая кредитная политика и переписывание телефонии в объём не входят. Админка сохраняет существующее английское представление. По уточнению от 10.09.2026 исключение записи на приём заменено отдельным ограниченным расширением в §10: одна запись по предварительному разрешению.

Пакеты ниже сохраняют исходную структуру реализации и критерии приёмки. Они не требуют повторной реализации. Текущее поведение описывает [архитектура](architecture.md), единый остаток работ — [roadmap](mvp-plan.md). Исторические числа тестов относятся к указанным датам.
...
- [x] Уточнить P05–P07: сведения утверждаются до звонка; неизвестное можно уточнить; абсолютного «не угадывает» нет. Первоначальный информационный сценарий записи от 09.09 расширен 10.09 по §10; новые исходники текстов подготовлены, их приёмка и CMS-публикация остаются отдельными. Не показывать mock live approvals как функцию настоящего звонка.
...
| Пример обращения в приёмную, уточнение 10.09 | Ask about appointments and documents, or book one appointment within the limits you approve beforehand. | Nach Terminen und Unterlagen fragen oder einen Termin innerhalb Ihrer vorab freigegebenen Vorgaben vereinbaren. |
...
## 10. Расширение от 10.09.2026 — одна запись по предварительному разрешению

Это уточнение заменяет прежнее ограничение «только выяснить доступное время». Оно не добавляет запрос разрешения пользователю посреди разговора. Реализованы подготовка, утверждение и исполнение ограниченного разрешения; новая живая телефонная приёмка и публикация CMS остаются отдельными шагами.

В утверждаемом плане пользователь разрешает одну из операций: `book` или `confirm_existing`. Разрешение задаёт услугу, именно вызванного получателя и часовой пояс IANA. Для `book` допустимо до 31 окна начала записи с полной датой и временем: обе границы включены, одинаковые границы означают точное время. Для `confirm_existing` требуется ровно одно окно с одинаковыми границами — известные дата и точное время существующей записи. При новой записи выбирается первый предложенный получателем вариант, который укладывается в эти рамки; поиска самого раннего свободного времени не обещаем. Перенос, отмена, платежи, депозиты и новые финансовые условия либо штрафы за отмену не разрешены. Исторический план без явного разрешения таких полномочий не получает.

Перед согласием на конкретный вариант модель вызывает `check_appointment` с полными датой, временем, часовым поясом и явными утверждениями о соответствии услуги, получателя, отсутствии новых условий и подтверждённых деталях. Сервер проверяет их относительно утверждённого разрешения. Пропущенные поля, иной scope, прошедшее время, несуществующее время при переходе часов и неоднозначное время отклоняются. Эти поля остаются утверждениями модели о разговоре, а не независимой проверкой учреждения.

Успех проверки разрешает действие, но не означает «записано». После запроса записи/подтверждения нужна явная ответная реплика получателя; без неё результат остаётся неподтверждённым. Сводка опирается на исходную расшифровку и её ссылки. Доступа к календарю учреждения и calendar API нет. Счётчик принятого действия хранится в текущем Realtime-соединении; повторные события внутри него не повторяют разрешение. Возобновление разговора после разрыва не реализовано: перед добавлением reconnect потребуется сохранять этот счётчик на уровне попытки. Порядок реплик и ответ кнопкой проверяются сервером, смысл подтверждения — моделью по разговору.

Исходники EN/DE обновлены в `seed-content.ts`: карточка приёмной, Terms/AUP, FAQ page и FAQ collection используют согласованные ограничения. Генератор release candidate включает новые тексты и reason, сохраняя прежний идентификатор совместимого staging workflow `public-copy-2026-09-09`; он не обозначает дату содержимого. Состояние существующей CMS от этого не меняется: черновики от 09.09 нужно сверить с новыми текстами через [обычный staging workflow](public-content-staging-2026-09-09.md), сохранив исходные revisions и прежние acceptances.

Приёмка расширения:

- [x] Чистый серверный validator: 82/82 теста, включая точные/раздельные окна, неизвестное разрешение, несовпадающие параметры, новые условия, прошлое, переходы часов на час и полчаса, часовые пояса со сдвигом на четверть часа. Это проверка переданных данных, не e2e звонок.
- [x] Проверены новые версии compilation/execution, hash/review, перевод разрешения и чтение legacy без расширения полномочий. Новый writer после исправления подготовки: schema 4 / compiler 5 / policy 3 / execution 2; compiler 3 и 4, legacy execution 1 читаются с прежним hash. SQL-миграций для расширения нет.
- [x] Runtime gate проверен: одно принятое действие в текущем разговоре, повтор tool call не повторяет разрешение, отклонённая проверка его не расходует, второй вариант после принятого не разрешается.
- [x] Проверены прерывания, stale/cancelled/parallel tool calls, ответ DTMF и ожидание нового ответа после разрешения. Сводка на реальном текстовом provider различает обрыв до подтверждения (`unknown`) и явную запись со слов получателя (`reported` со ссылкой на его реплику).
- [x] EN/DE review визуально проверен на реальных компонентах со статическими вымышленными данными: desktop и 390 px; исправлено выравнивание нескольких окон. Изменение перевода сохраняет параметры разрешения. Это не авторизованный браузерный e2e и не телефонный звонок.
- [ ] Провести живую голосовую приёмку новой записи и подтверждения существующей: подходящий/неподходящий слот, новые условия, обрыв до подтверждения и завершение агентом.
- [ ] Сверить актуальные CMS drafts, обе FAQ-поверхности и Terms/AUP, проверить preview и решение о reacceptance; публиковать только через действующий workflow после готовности runtime. R06–R14 остаются отдельными условиями выпуска.

Проверки 10.09: полный API на новой изолированной PostgreSQL БД — 684/684 без пропусков; после последних трёх регрессий компилятора его suite повторён отдельно — 82/82. Contracts — 92/92, web — 165/165. Typecheck, web lint, обе production-сборки, `copy:check` и `git diff --check` прошли. Временная тестовая БД удалена, прежние рабочая и тестовая базы не изменялись. Логи и визуальные fixtures находятся только в ignored `.tools`.

Реальный текстовый provider проверен на вымышленных заданиях: новая запись RU, подтверждение DE/EN, отсутствие временных рамок и запрос только доступности. Две выявленные ошибочные блокировки (формат календарной даты и формулировка `confirm my attendance`) исправлены и закреплены сохранёнными синтетическими регрессиями; RU-запись также повторно прошла запрос provider. Локальное приложение обновлено, API readiness и тоннель проверены; автоматическое завершение звонка включено. Реальных телефонных звонков в этой проверке не было.

Уточнение подготовки встреч от 10.09: проверка намерения по ключевым словам заменена структурированной интерпретацией модели с цитатой из задания или ответа на уточнение. Сервер рассчитывает конкретные окна из календарного правила: даты/диапазон, следующие N дней или следующая календарная неделя, дни недели, исключения и отдельные интервалы начала. Расчёт привязан к одному текущему времени и часовому поясу, не к языку интерфейса. Исключённая календарная дата может быть объяснена в плане, но не попадает в разрешённые окна. Защита остальных имён, контактов и идентификаторов сохраняется. Полномочия по-прежнему появляются только после утверждения плана пользователем.

Личная встреча подтверждается явным согласием нужного человека на согласованные детали; наличие организации, услуги или записи в календаре для неё не требуется. Неверно сформированный план повторно генерируется в рамках существующего лимита. Если исправить его не удалось, интерфейс показывает ошибку подготовки и позволяет повторить сохранённый запрос. Для действительно недостающих рамок остаётся обычное уточнение. Старые ревизии не переписываются; повтор подготовки создаёт новую.

Проверка этого уточнения: 12/12 вымышленных заданий прошли реальный текстовый provider — RU/DE/FR/UK/IT/EN, личная встреча, следующая календарная неделя, исключённые дни/даты, раздельные интервалы, сегодняшнее окно, подтверждение существующей встречи, запрос только доступности и уточнение недостающих рамок. Contracts — 92/92, web — 173/173. Полный API на отдельной PostgreSQL БД: 772/773; единственная устаревшая fixture исправлена добавлением обязательного промежуточного ответа модели, затем её suite повторён на новой БД — 25/25. Обе временные БД удалены. После уточнения о неповторении уже полученного ответа Realtime suite — 43/43; API typecheck, web lint/typecheck, обе сборки и проверки текста/whitespace прошли.

Исходный пользовательский кейс повторно подготовлен через обычную кнопку интерфейса: новая ревизия 4 получила `ready_for_review`, исходный запрос с уточнениями сохранён, контрольные суммы всех четырёх ревизий валидны. Пользовательское утверждение не выполнено. API обновлён, readiness — 200; действующий тоннель отклоняет неподписанный webhook ответом 403. Это подтверждает подготовку и технические проверки, но не заменяет живую голосовую приёмку.

Уточнение устойчивости подготовки от 10.09: отдельный технический сбой показал, что прежние 25 секунд на генерацию могут обрывать корректное задание, а браузер через две минуты ошибочно сообщает о небезопасности. По умолчанию генерация теперь получает до 60 секунд на запрос, модерация — до 25 секунд; общий срок одной попытки worker — 120 секунд с резервом для финальной модерации. Повторные запросы и чтение тела ответа ограничены оставшимся сроком, суммарный лимит восьми provider requests сохранён. Явные значения env продолжают переопределять defaults.

Интерфейс различает ожидание, технический сбой и отказ проверки содержания. Повтор неизменённой формы или ответов на уточнения проверяет прежнюю подготовку с тем же ключом. Новая подготовка разрешается только после подтверждённого конечного сбоя; за одну явную повторную отправку создаётся максимум одна замена. Смена UI-языка и недоступная запись в sessionStorage учтены. Эти повторы не утверждают план и не запускают звонок.

Проверка сроков и восстановления: compiler suites — 175/175, web — 184/184, call-service и durable jobs — 38/38 на новой изолированной PostgreSQL БД; временная БД удалена. Дополнительный вымышленный сценарий с реальным provider получил `ready_for_review` за 95 секунд: после первого timeout повторная генерация заняла 33 секунды и прошла финальную модерацию; окна и исключённый день проверены. Это подтверждает восстановление данного сценария, но не гарантирует время ответа внешнего сервиса.

API и web production-сборки, typecheck/lint, `copy:check` и `git diff --check` прошли. API перезапущен после завершения активного звонка, с сохранением голосового завершения и настроек текстового provider. Readiness и web — 200; неподписанный запрос через тоннель — ожидаемый 403. Голосовой звонок для этой проверки не запускался.

Последующий реализованный пакет после аудита реального разговора: [стабилизация речи, прерванного прощания и краткого итога](call-conversation-result-stabilization-plan-2026-09-10.md). C01–C06 реализованы; следом выполнены [общие исправления A01–A03](call-result-live-fixes-2026-09-11.md). Полная голосовая приёмка новых инструкций остаётся открытой. Завершение соединения получателем первым считается нормальным; исправления относятся к служебным репликам и качеству представления результата.

Уточнение пользователя для текущего локального этапа: обратная совместимость старого summary не требуется. Пакет C04–C05 переводится на единый новый контракт с кратким итогом и подтверждающими facts/findings; старые тестовые summary и их задания очищаются однократной процедурой перед запуском нового reader. Исходные расшифровки, утверждённые данные и версии генерации сохраняют свои функции проверки корректности. Это решение заменяет прежнее требование одновременно поддерживать summary v1/v2 в плане стабилизации; локальный переход выполнен 10.09.2026: удалено содержимое прежних тестовых summary/частей, старые задания отменены; hashes, учёт, исходные расшифровки, планы, утверждения и переводы сохранены. Это не удаление всех исторических форматов продукта: совместимость compilation/approval/ciphertext остаётся, как описано в архитектуре.
