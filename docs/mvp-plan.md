...
- **PARTIAL — async work:** final transcription, recording retention, and Twilio call/recording status reconciliation now run through PostgreSQL-backed durable jobs with leases, fencing, bounded retries, attempt history, dead-letter state, and restart seeding. Webhook delivery visibility and a separately deployed worker runtime remain open.
...
Current acceptance: stale call and recording state is durably detected and reconciled without weakening signed webhook boundaries; restart recovery no longer invents a Twilio failure/refund before querying provider state; and pre-connection busy/no-answer paths still refund rather than charge. Full 6A2 acceptance remains open until operators can distinguish delayed webhook delivery from terminal provider failure and the split API/worker deployment boundary is proven.
