...
- **PARTIAL — async work:** final transcription, recording retention, and Twilio call/recording status reconciliation run through PostgreSQL-backed durable jobs with leases, fencing, bounded retries, attempt history, dead-letter state, restart seeding, and an independently runnable worker. Real-provider crash drills are complete; cross-process live events and externally monitored worker liveness remain open.
