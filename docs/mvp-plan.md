...
The next repository checkpoint is **6F4 — password/account recovery**: define the anti-enumeration and abuse model, require verified-phone OTP step-up, issue a short-lived single-use recovery grant, replace the password with scrypt, revoke every existing session, append minimized immutable evidence, add distributed-ready rate-limit boundaries, and deliver localized EN/DE recovery screens and tests. The production backup/deletion-replay gate remains tracked separately for deployment hardening.
