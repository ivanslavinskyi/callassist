...
- **PARTIAL — production security:** bounded HTTP requests, API/web security headers, global unsafe-origin rejection, hardened production cookies, fail-closed API/worker configuration, migration drift detection, a high-severity dependency audit gate, Dependabot configuration, and repository CI are implemented; external CI execution/branch protection, managed secrets, infrastructure controls, restore testing, focused security/privacy review, and deployment evidence remain.
