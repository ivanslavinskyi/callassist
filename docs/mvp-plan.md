...
- **PARTIAL — data lifecycle:** recording deletion, transcript export, versioned full-data export, current logout, bounded active-session inventory, owner-scoped selective revocation, tested self-service all-session revocation, verified-phone password recovery/change, password-confirmed terminal-call deletion, and durable account-wide deletion/anonymization with support recovery exist; production backup expiry and restore-time deletion replay evidence do not.
- **PARTIAL — identity foundation:** user/session tables, repositories, shared contracts, scrypt password handling, register/verify/resend/login/logout/me/recovery/verified-phone-change/session-inventory/selective/all-session-revoke/data-export endpoints, immutable minimized session/export/recovery/phone-change evidence, localized register/verify/login/recovery/account screens, Twilio Verify integration, opaque server-side session cookies, credentialed web API requests, server-side app/admin route guards, and shared PostgreSQL auth/public-safety/expensive-endpoint limits exist. Suspicious-activity triggers, reusable recent step-up, security-change notification, and support-assisted phone-loss recovery remain.
...
## Completed checkpoint — verified phone-number change and re-verification (6F6)

- [x] Add strict authenticated start/confirm contracts and allowed-origin routes. Start requires the current password; repository creation rechecks the verified password hash under the same user/session lock so a concurrent reset cannot authorize stale state.
- [x] Bind a 10-minute, eight-attempt durable challenge to the initiating user and exact active session, send/check OTP only against the replacement number, invalidate an earlier challenge on restart, and invalidate the challenge when provider delivery fails.
- [x] Atomically replace and verify the unique new number, preserve only the initiating session, revoke every other active session, invalidate unused password-recovery challenges/grants created for the old phone, invalidate other pending phone changes, and reject replay.
- [x] Preserve uniqueness under concurrent completion by different accounts; the challenge never reserves a number and the users-table constraint allows exactly one claimant to commit.
- [x] Add shared IP/user/phone/challenge abuse limits plus the durable attempt cap. Store no phone in immutable evidence or operational errors; delete temporary challenge phone data after 30 days without deleting the phone-free event.
- [x] Deliver the localized EN/DE Account flow with password → SMS → completion state held only in React memory, update the verified profile in place, and remove revoked sessions from the visible inventory.
- [x] Cover strict payloads, session/cross-user isolation, occupied/same number, wrong password/code, provider failure, replay, recovery invalidation, retention, memory/PostgreSQL parity, and concurrent unique-number claims.
...
- [ ] Define re-verification for number changes and step-up verification for recovery, suspicious activity, and sensitive operations. **Partial:** verified-phone recovery and session-bound phone replacement use durable OTP attempts and atomic credential/session invalidation; current-password step-up protects deletion and phone change. Suspicious-activity triggers, reusable recent step-up grants, security-change notifications, support-assisted phone-loss recovery, and the complete sensitive-operation matrix remain open.
