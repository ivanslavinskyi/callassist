...
- **PARTIAL — production security:** bounded HTTP requests, API/web security headers, global unsafe-origin rejection, hardened production cookies, fail-closed API/worker configuration, migration drift detection, a high-severity dependency audit gate, Dependabot configuration, repository CI, and a disposable local database recovery drill are implemented; external CI execution/branch protection, managed secrets/backups, infrastructure controls, an isolated production restore, focused security/privacy review, and deployment evidence remain.
...
- [ ] Secrets/keys managed with least privilege and rotation procedure. **Partial:** secret-family procedures and the data-key re-encryption constraint are documented; the managed store, named access policy, dual-read/key-version support and exercised rotation evidence remain.
