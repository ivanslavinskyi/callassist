...
- **PARTIAL — data lifecycle:** recording deletion, transcript export, versioned full-data export, current logout, bounded active-session inventory, owner-scoped selective revocation, and tested self-service all-session revocation exist; transcript/call-data deletion and account deletion/anonymization do not.
...
The next repository checkpoint is **6F3 — data and account deletion/anonymization**: define the record-by-record lifecycle policy first, including transcript/call-data scope, immutable financial/consent/audit evidence, recipient suppression continuity, Twilio recording deletion, in-flight durable jobs, backup expiry and support escalation; then implement owner requests as idempotent durable work with explicit status and failure recovery.
...
- [ ] Account/data export/delete and session revocation tested. **Partial:** bounded session inventory, current/selective/all-session revocation, and versioned full-data export are exposed in `/app/account`, owner-isolated, rate-limited, immutably evidenced and tested; data deletion and account deletion/anonymization remain.
