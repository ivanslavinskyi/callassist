...
The next repository checkpoint is **6F5 — shared rate limits and abuse controls**: replace process-local buckets with one atomic cross-instance store, preserve hashed identifiers and bounded cardinality, define fail-closed/fail-open behavior per endpoint, add retry/expiry semantics and privacy-safe metrics, and prove registration/login/recovery/costly-endpoint limits across multiple API instances. The production backup/deletion-replay gate remains tracked separately for deployment hardening.
