...
- **PARTIAL — async work:** final transcription, recording retention, and Twilio call/recording status reconciliation run through PostgreSQL-backed durable jobs with leases, fencing, bounded retries, attempt history, dead-letter state, restart seeding, and an independently runnable worker. Cross-process live events and real-provider crash drills remain open.
