...
- **PARTIAL — observability:** audit/provider/SSE/health data, a privacy-safe durable technical event stream, versioned outcomes, owner feedback, Admin Calls/Inspector, operational cohorts/cost estimates, local system controls, and bounded webhook-delivery evidence exist; upstream probes, production monitoring/alerting, and invoice reconciliation remain.
- **PARTIAL — async work:** final transcription, recording retention, and Twilio call/recording status reconciliation now run through PostgreSQL-backed durable jobs with leases, fencing, bounded retries, attempt history, dead-letter state, and restart seeding. A separately deployed worker runtime remains open.
...
## In-progress checkpoint — provider and webhook reconciliation (6A2)
...
Current acceptance: stale call and recording state is durably detected and reconciled without weakening signed webhook boundaries; restart recovery no longer invents a Twilio failure/refund before querying provider state; pre-connection busy/no-answer paths still refund rather than charge; and operators can distinguish callback acceptance, rejection, missing targets, and processing failure without exposing call/provider IDs or payloads. Full 6A2 acceptance remains open until the split API/worker deployment boundary and real-provider crash behavior are proven.
...
- [ ] `/admin/system`: API/DB/Twilio/OpenAI health, active calls, jobs/failures, webhooks, retention, transcription, costs, alerts, and kill switch. **Partial:** API/database request-path health, provider configuration (not upstream health), active workload, durable queue/retry/dead-letter detail, privacy-safe webhook-delivery aggregates and age, recent warning/error counts, costs via the overview, reasoned superadmin job recovery, and RBAC kill-switch control exist; provider probes and production alerts remain.
...
- [ ] Use durable idempotent jobs for final transcription, retries, recording processing, retention deletion, provider reconciliation/status sync, and cleanup. **Partial:** PostgreSQL-backed transcription, retention, and provider call/recording reconciliation now provide leases, fencing, bounded retry/dead-letter state, immutable attempts, transactional scheduling, and restart seeding; remaining generic cleanup and a separately deployed worker remain.
- [ ] Test restart, duplicate webhook, concurrent worker, and partial provider failure; expose stuck/dead-letter work. **Partial:** lease-expiry recovery, stale-worker fencing, duplicate enqueue, concurrent PostgreSQL claims, bounded retry, dead-letter visibility, audited manual restart, and deterministic lost call/recording callback recovery are covered; real Twilio partial-failure/crash drills remain.
