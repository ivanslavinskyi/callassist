...
- Twilio: исходящий PSTN-звонок и двунаправленный Media Stream.
...
При перезапуске API незавершённые звонки переводятся в `failed`, ожидающие подтверждения истекают, а восстановление фиксируется в аудите. Межпроцессный event bus и фоновые повторы появятся вместе с Redis/BullMQ перед реальной телефонией.
