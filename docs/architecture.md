...
Initial call-brief preparation carries a browser-generated UUID idempotency key. The browser retains that key while retrying an unchanged form after an uncertain response, and replaces it only after success or a form edit. The service returns a completed matching request before recompilation; a partial unique PostgreSQL index remains the atomic race boundary. SSE subscriptions attach cleanup to the HTTP response rather than the already-consumed request, emit heartbeats, disable common reverse-proxy buffering, and rely on the normal snapshot read after reconnect for recovery.
