...
- Identity and tenancy foundation: PostgreSQL users and revocable server-side sessions, scrypt password hashes, explicit first/last names, localized registration/verification/login/recovery screens, Twilio Verify phone confirmation, and opaque HttpOnly session cookies. Verified-phone password recovery gives every valid email the same public start response, durably caps a 10-minute challenge at eight OTP attempts, stores only the digest of a distinct 15-minute single-use grant, and atomically changes the password, revokes all sessions, consumes the grant, and appends immutable minimized evidence. Login session creation is locked to the password hash that was actually verified so an old-password race cannot survive a concurrent reset. Authenticated phone replacement requires the current password and an eight-attempt, 10-minute OTP challenge bound to the initiating session; atomic completion verifies a unique new number, keeps only that session, invalidates old-phone recovery capabilities, and stores phone-free immutable evidence. The authenticated account boundary exposes a maximum of 50 active sessions with only categorized browser/platform and lifecycle timestamps; raw User-Agent, token hash, and IP never leave the API. Single-session revocation is owner-scoped, current-session revocation clears the cookie, and selective/all-session changes append immutable minimized security events. New call briefs store their authenticated owner. Every browser call list/read/write/action/SSE/media route authenticates the session and checks that owner without accepting a browser-supplied user ID; provider webhooks keep their independent signature/provider-ID boundary. Narrow admin/superadmin account-control routes atomically suspend/unsuspend users or revoke their sessions with a mandatory reason and immutable audit event. Suspension also blocks session creation and PostgreSQL call creation/reservation under row locks; unsuspension never restores revoked tokens. Paginated administrator search exposes only name, email, role/status, verification state, and activity dates before a separately authorized ledger read; it excludes phone numbers, password hashes, and session credentials. The selected-user console composes the narrow status, session-revocation, and idempotent credit-grant APIs, requires reasons and destructive confirmations, and mirrors server eligibility rules without replacing them. Ordinary admins can discover only `user` accounts, while superadmins may inspect staff roles. Pre-authentication rows remain hidden pending an explicit archive/backfill decision.
...
- Request-abuse boundary: registration, authentication and recovery actions, public recipient opt-out, and expensive brief preparation, call start, promo redemption, recording download, export/deletion, and transcription retry endpoints use fixed-window application limits. PostgreSQL-backed deployments share atomic cross-instance buckets keyed by an independent HMAC-SHA-256 digest; raw email, phone, IP, user, recovery, and token identifiers are never stored. Grouped user/IP and phone/IP decisions increment all buckets or none, return `Retry-After`, expire within seven days, and fail closed at a 100,000-bucket cap or store outage before privileged/provider/cost side effects. Eligibility-sensitive recovery start remains publicly generic while suppressing SMS. Thirty-day hourly allowed/denied totals contain controlled scopes only and surface as a privacy-safe 24-hour Admin System view. Recovery also retains its durable eight-attempt challenge cap. The in-memory implementation is limited to tests and single-process development.
- Twilio: outbound PSTN calls, signed webhooks, a bidirectional Media Stream, optional keypad consent fallback, and temporary consent-gated recordings. REST Call resource statuses and progress-webhook `CallStatus` values are modeled separately because the callback-only `initiated` value maps to domain `dialing` but is not returned as a REST resource status.
...
The API depends on a `CallRepository` interface. The in-memory and PostgreSQL repositories implement the same contract, keeping telephony and Realtime independent from storage. SQL migrations are versioned with the API. An owner-scoped encrypted preparation request is persisted with a `brief_compilation` job before any model call, and only a fenced worker may publish the resulting call brief.

Initial call-brief preparation carries a browser-generated UUID idempotency key. A user-scoped browser session record stores no raw form data and binds that key to a SHA-256 fingerprint of the normalized form, so an unchanged operation survives component remounts and uncertain HTTP responses. `POST /api/call-preparations` validates and rate-limits the request, then atomically persists its encrypted normalized input together with a `brief_compilation` durable job and returns `202` without calling OpenAI. Owner-scoped status reads expose only bounded state, attempt count, controlled terminal failure, and the resulting call ID. The API client recovers one uncertain enqueue response with the same key, polls with a hard deadline, and loads the owner-authorized brief snapshot before navigation. A fenced worker uses the same configured compiler in embedded and standalone modes; call insertion, successful preparation state, and private-input erasure commit in one repository transaction. Retries after a crash reuse the call-creation key, so at-least-once execution still converges on one brief. Terminal requests wipe encrypted input. Synchronous HTTP creation through `POST /api/call-briefs` does not exist. Account deletion cancels queued/running preparation jobs, records cancellation of an active attempt, erases source input, and locks publication against the deletion request. Admin System reports the preparation queue and controlled job state without exposing its source payload or offering an impossible dead-letter retry. SSE subscriptions attach cleanup to the HTTP response rather than the already-consumed request, emit heartbeats, disable common reverse-proxy buffering, and rely on the normal snapshot read after reconnect for recovery.
...
are saved in SHPROHLI, all recipients are addressed formally unless the operator
...
The Twilio voice webhook immediately opens a bidirectional Media Stream with call recording disabled. A separate short-lived consent session uses the selected voice to disclose the AI identity and represented person, explain recording and transcription, and ask for consent verbally. If the spoken answer remains unclear after clarification, the assistant offers keypad confirmation. Before consent is confirmed, the server isolates recipient media from the main conversation and does not record it.
